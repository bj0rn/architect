...
func getCompleteVersion(appversion AppVersion, buildImage BuildImage, baseImage BaseImage) string {
	return fmt.Sprintf("%s-b%s-%s", appversion,
...
givenVersion string, buildImage BuildImage, baseImage BaseImage) *AuroraVersion {
	return &AuroraVersion{
		...
		Snapshot: snapshot,
		givenVersion: GivenVersion(givenVersion),
		completeVersion: getCompleteVersion(AppVersion(appVersion), buildImage, baseImage),
	}
}
...
func (m *AuroraVersion) getVersionTags(extraTags config.PushExtraTags) ([]string, error) {
	...
	if m.isSemanticReleaseVersion() {
		...
		versions = append(versions, m.completeVersion)
	}
	return versions, nil
}
